...
`.static_call_sites`、`__mcount_loc`(ftrace)写回代码的是指向**区域外**的
`call rel32`,偏移随函数位置而变 —— 与"函数体内不得有指向区域外的 PC 相对引用"
这条不变式直接冲突,**无解**。含这两类条目的函数不能进入随机化范围
(`scripts/ikaslr/gen_funcs.py` 的硬约束)。

> 这条同时回答了 E3-10/R-67 的一半:ftrace 的插桩点与被随机化的函数体不能共存,
> 除非把 `call __fentry__` 也改成绝对间接调用。
...
### 6.1b 一个必须记下来的规模结果:大范围下区域几乎等不到空

同一套并发压力测试,把范围从 S1(31 函数)换到 S3(483 函数,覆盖 VFS/mm/net):

| 范围 | 触发次数 | 成功轮数 | `enter_backoffs` |
|---|---:|---:|---:|
| S1 = 31 函数 | 400 @ 2 ms | 250–390 (**~70%**) | ~1.5 千 |
| S3 = 483 函数 | 200 @ 20 ms | 0–6 (**0–3%**) | ~9 **百万** |

**正确性完全不受影响**:两种范围下 `whitelist_rejects`、`stale_fixup_fails` 都是 0,
无 oops,自检全过。受影响的是**活性**:范围一大、负载一重,活跃集合就很难在
`IKASLR_WAIT_MS`(100 ms)内变空,随机化成批地跳过本轮 —— 这是设计里既有的安全降级
路径(`region not empty within 100 ms, skipping round`),不是缺陷。

代价体现在 `enter_backoffs` 上:阻断窗口期间每个想进入区域的执行流都要"加一 →
看见阻断 → 减一 → 等 → 重试"。窗口长达 100 ms 时这个循环会跑上百万次。

这直接说明了实验清单里 **E3-4(无拷贝)与 E3-6(延迟触发)的必要性** —— 它们正是
为了绕开"必须等到区域完全为空"这一条。也说明性能实验里
`TRIGGER_MS` 与 `IKASLR_WAIT_MS` 的取值必须一并报告,否则数字无法解释。
...
### 7.8 ⚠️ 大范围下的活性(**待定,影响实验设计**)

见 §6.1b。S3 范围 + 并发负载下随机化成功率只有 0–3%。可选的方向:
- 调小 `IKASLR_WAIT_MS`(失败得快一些,浪费的自旋少一个数量级,但成功率更低);
- 做 E3-4(无拷贝)/ E3-6(延迟触发),从机制上绕开"等区域全空";
- 缩小范围(但那削弱了第 6 章的覆盖面论证)。
