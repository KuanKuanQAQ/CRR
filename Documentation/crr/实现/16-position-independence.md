...
```
  base 内核，函数 F 编译在 .text 里（-mcmodel=kernel）

  ffffffff81ec7a00
  +--- F -----------------------+
  |   jne   +0x58          --(1)|      (1) 函数内分支   目标在框内
  |   call  rel32          --(2)|      (2) 跨函数调用   目标在框外
  |   mov   off(%rip),%rax --(3)|      (3) 全局变量     目标在框外
  |   ret                       |
  +-----------------------------+      三者的编码都是 PC 相对位移

  把这段字节原样复制到 vmalloc（距 .text 23-55 TiB）

  ffffc90000123000
  +--- F 的副本 ----------------+
  |   jne   +0x58          --(1)|      OK   源与目标一起位移，距离不变
  |   call  rel32          --(2)|      BAD  位移没变而 PC 变了
  |   mov   off(%rip),%rax --(3)|      BAD  落到一个无关的 vmalloc 页
  |   ret                       |
  +-----------------------------+
```

**位置无关要解决的就是 (2)(3):把"到目标有多远"换成"目标在哪"。**
...
```
  ...
  +-----------------------------+
            ...
            |  LLVM pass（OptimizerLast 挂点）
            |    (1) 新建同名同签名的跳板
            |    (2) RAUW：所有使用者改指向跳板   <- 必须先于改名
            |    (3) 原函数改名 F_body、换到 .rand.text.F
            |    (4) 物化 F_body 里所有指向区域外的引用
            v
  ...
  +--- .rand.text.F ------------+
  | F_body:                     |      会被搬移的那一块
  |   jne   +0x58               |        内部分支：仍是 PC 相对
  |   movabs $cb,%rax           |        外部引用：改成 64 位绝对
  |   call  *%rax               |
  |   movabs $g_var,%rax        |
  |   mov   (%rax),%rax         |        取值要多一次解引用
  |   ret                       |
  +-----------------------------+
```
...
```
  x86-64：绝对地址就在指令的立即数里

  +--- .rand.text.F ---------------------------+
  | F_body:                                    |
  |   48 b8 b0 8d 0b 81 ff ff ff ff            |   10 B，R_X86_64_64
  |        movabs $0xffffffff810b8db0,%rax     |
  |   ff d0                                    |    2 B
  |        call   *%rax                        |
  +--------------------------------------------+
                                          立即数与本指令的位置无关
  ...
  AArch64：没有 64 位立即数，绝对地址放进随函数搬移的字面量池

  ffff8000810dc720
  +--- .rand.text.path_get -----------------+
  | path_get_body:                          |
  |   +0x18  ldr  x0, <+0x78>               |   取出 mntget 的地址
  |   +0x1c  ldr  x8, <+0x80>               |   取出 ikaslr_out_enter 的地址
  |   +0x20  blr  x8                        |   fixed_out：白名单检查 + 记账
  |   ...                                   |
  |   +0x70  ret                            |
  |   - - - - literal pool - - - - - - -    |   <- 在 ELF 符号大小之外
  |   +0x78  .quad  ffff80008031783c        |   mntget
  |   +0x80  .quad  ffff8000800c50e4        |   ikaslr_out_enter
  |   +0x88  .quad  ffff80008031783c        |   mntget
  |   +0x90  .quad  ffff8000800c5220        |   ikaslr_out_leave
  +-----------------------------------------+
       指令里的 <+0x78> 与池中的 +0x78 是同一处：都是节内偏移，
       随函数整体搬移而不变。池项是数据，R_AARCH64_RELATIVE，
       由加载期重定位填成绝对地址。
```

#### 图 4　搬移时,三类引用各自为什么仍然正确

```
  母本 .rand.text.F                 变体（vmalloc，位置随机）
  +------------------+              +------------------+
  | F_body:          |  逐字节复制  | F_body':         |
  |   jne   +0x58    |  ==========> |   jne   +0x58    |  OK  相对，随体搬移
  |   movabs $cb,%rax|              |   movabs $cb,%rax|  OK  绝对，与位置无关
  |   call  *%rax    |              |   call  *%rax    |
  +------------------+              +--------^---------+
                                             |
  __ikaslr_target_F  ------------------------+
                                   只改这一个指针 -> O(函数数)，不是 O(调用点数)
```

| 引用种类 | 编码 | 搬移后为什么对 |
|---|---|---|
| 函数**内部**分支 | PC 相对 | 源与目标同属一个函数体,一起位移,**距离不变** |
| 指向区域**外** | 绝对(物化) | 绝对地址**与指令位置无关** |
| 外界**进入**本函数 | 经 `target` 槽 | 随机化时改槽,每函数一次 |

**"函数粒度"这个选择的全部根据就在第一行**:以整个函数为单位搬,
内部相对关系全都不变,于是只需处理跨越函数边界的引用。
...
### 4.1.1 为什么不是"把 call 改成绝对地址 call"

这是读到这里最自然的疑问,答案是:**x86-64 没有这条指令。**

| 编码 | 形式 | 能否用 |
|---|---|---|
| `E8 rel32` | 近调用,**相对位移** | 就是要消灭的那个 |
| `FF /2` | 近调用,**间接**(寄存器或内存) | ✅ 唯一能到任意 64 位地址的路 |
| `9A ptr16:32` | 远调用,绝对立即数 | **64 位模式下是无效操作码** |
| `FF /3`(`lcall`) | 远调用,经内存的 `m16:64` | 存在,但换 CS、走段描述符、要 `lretq` 返回,与 C ABI 不兼容且极慢 |

所以 `movabs $addr,%rax; call *%rax` **就是**绝对地址调用 —— 不是退而求其次,
而是这条语义在 x86-64 上唯一的表达方式,ISA 要求它占两条指令。
arm64 同理:`bl` 是 ±128 MB 的 PC 相对,`blr Xn` 是唯一能到任意地址的形式。

### 4.1.2 那"保留 rel32、随机化时按变体改写位移"呢

另一个合理的想法,但**量程差四个数量级**。变体分配在 vmalloc 区
(`kernel/ikaslr/randomize.c`,`__vmalloc_node_range(..., VMALLOC_START, VMALLOC_END, ...)`):

```
_stext − VMALLOC_START = 0x36ff81000000 = 55.0 TiB
_stext − VMALLOC_END   = 0x16ff81000001 = 23.0 TiB
rel32 的量程           =                   ±2 GiB
```

**最近处也超出量程 11 775 倍**,位移编不进那 4 个字节。

这个坑踩过一次:`text_poke_bp()` 的 `struct text_poke_loc.rel_addr` 是 s32
(相对 `_stext`),正因如此它无法给变体打补丁 —— `movslq` 出来一个截断的地址,
直接缺页。那次的教训催生了现在"改母本 + READY,再切变体"的设计。

即便把变体放进 module 区(在 ±2 GB 内、rel32 够得着),也不该这么做:

| | S3 |
|---|---:|
| `target` 槽(现在每轮要更新的) | 496 |
| 区域内的间接调用点 | 4 319 |

索引更新会从 **O(函数数)** 退化成 **O(调用点数)**;更要命的是
**"变体永远从母本逐字节复制"这条不变式会消失** —— 每造一个变体都要重写几千处位移,
而那条不变式正是避开与陷阱投毒竞态的关键。

### 4.1.3 x86 上还有第三条路,只是不划算

也可以学 arm64,用 `call *disp32(%rip)` 指向**同节内、随函数搬移**的池项:

```
call *disp32(%rip)    6 字节  + 池项 8 字节（每个不同符号一份）
movabs + call *%rax  12 字节  （每个调用点一份）
```

同一个符号在一个函数里被调 2 次就开始赚(8+2×6=20 vs 2×12=24)。
这就是 §11 里列为"未做"的**共享偏移表**:x86 上它纯属体积优化,不是刚需;
arm64 上因为没有 `movabs` 可用,它是刚需。
...
**注意:卡住的不是"拼不出来",而是"填不进去"。** 四条指令 16 字节是拼得出
一个 64 位数的 —— 如果那是个**编译期常量**(`mov x0, #0x1234...`),完全正常。
问题出在要填的是**符号地址**:那需要 `R_AARCH64_MOVW_UABS_G0_NC` ~ `G3`
四条重定位,而**内核映像以 PIE 链接**(`CONFIG_RELOCATABLE`),链接器直接拒绝:

```
relocation R_AARCH64_MOVW_UABS_G0_NC ... can not be used when making a shared object
```

理由与 §5.2.1 是同一条:**没有任何动态重定位能在加载期修改指令的立即数字段。**
所以"符号地址 + PIE"这个组合无解,而"编译期常量"没事。

而 `adrp`(PC 相对的页寻址)恰恰是搬移后失效的那种寻址,更不能用。
**两条路都堵死了。**

顺带:即便 PIE 允许,字面量池也更省:
...
同一符号用一次 12 B vs 16 B,用两次 16 B vs 32 B —— 池子只会更小。
...
| 项 | 说明 |
|---|---|
| **arm64 共享 GOT** | 当前是**每函数一个字面量池**,同一个符号在多个函数体里各存一份。设计要的是共享表。作者 2026-09-10 决定不做 —— 注意它不只是工作量:`ldr`(literal)的偏移是 `imm19 × 4`,量程只有 **±1 MB**,而函数体会被搬到 vmalloc 区,一张全局共享表很容易超出这个量程,必须换成别的寻址形式。当前方案把池子放在函数体同节、紧随其后,天然落在量程内 |
