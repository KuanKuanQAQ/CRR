...
\lstset{basicstyle=\ttfamily}% 行内代码与正文同字号；代码块各自指定字号，不受影响
%- 图表标题：一律居中，多行标题不再按悬挂缩进左对齐
\captionsetup{justification=centering,singlelinecheck=false,hangindent=0pt}%
\captionsetup[bi-first]{justification=centering,hangindent=0pt}%
\captionsetup[bi-second]{justification=centering,hangindent=0pt}%
%- 浮动体排版参数：放宽单页可容纳的浮动体比例，减少整页浮动与大片空白
...
\renewcommand{\thealgorithm}{\thechapter-\arabic{algorithm}}% 算法按章编号，与图表一致
