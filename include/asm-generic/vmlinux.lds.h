...
/* 跳转桩区（M2）。由 link-vmlinux.sh 用 scripts/ikaslr/gen_stubs.sh 生成的
 * .tmp_ikaslr_stubs.o 提供；桩必须与 .rand.text 及运行期变体池同在 128 MB 直接
 * 分支量程内，故紧邻放置。文件内已 .balign 4096（严格 2^n 由 M6 观察点处理）。*/
#define IKASLR_STUBS							\
		. = ALIGN(PAGE_SIZE);				\
		KEEP(*(.ikaslr.stubs))
#endif
