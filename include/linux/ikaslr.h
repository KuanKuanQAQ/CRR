...
struct ikaslr_tramp {
	...
	void		*body;	/* 函数体（代码入口）的链接期原始地址 */
	void	       **target;/* x86 旧路径：target 槽。arm64（桩方案）不用，置 NULL */
	const char	*name;	/* 函数标识，用于调试与按名迁移 */
	size_t		 size;	/* 迁移单元字节数（含体前 12B 重入 prefix；初始化时填）*/
	int		 k;	/* arm64：名单下标，= 跳转桩 __ikaslr_stubs+8k 的序号 */
};
