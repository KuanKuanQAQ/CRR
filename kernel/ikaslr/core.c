...
/*
 * 间接调用离开区域：只做计数--（与 ikaslr_out_enter 相同），不查白名单——
 * 间接调用的目标是运行期值，静态白名单覆盖不到（属第 5 章 PA CFI 的范畴）。
 * 供 arm64 的 __ikaslr_indirect_out thunk 调用。
 */
...
EXPORT_SYMBOL_GPL(ikaslr_out_enter_indirect);
...
static int __init ikaslr_build_table_stubs(void)
{
	...
	ikaslr_tbl = kcalloc(n, sizeof(*ikaslr_tbl), GFP_KERNEL);
	if (!ents || !ikaslr_tbl) {
		kfree(ents);
		kfree(ikaslr_tbl);
		return -ENOMEM;
	...
	for (i = 0; i < ikaslr_ntramp; i++) {
		...
		unsigned long unit = (unsigned long)t->body - IKASLR_PREFIX_BYTES;
		unsigned long unit_end;

		/* 迁移单元 = [体入口−12, 下一单元起点)；末项到 .rand.text 末尾。*/
		if (i < ikaslr_ntramp - 1)
			unit_end = (unsigned long)ikaslr_tbl[i + 1]->body -
				   IKASLR_PREFIX_BYTES;
		else
			unit_end = (unsigned long)__rand_text_end;

		if (unit_end <= unit) {
			pr_err("k=%d bad layout (body=%px end=%lx)\n",
			       t->k, t->body, unit_end);
			return -EINVAL;
		}
		t->size = unit_end - unit;

		if (IS_ENABLED(CONFIG_IKASLR_DEBUG))
			pr_info("  [%d] k=%-4d body=%px size=%zu\n",
				i, t->k, t->body, t->size);
	}
	return 0;
}
