...
static int ikaslr_prepare(struct ikaslr_variant *v)
{
	...
	for (i = 0; i < ikaslr_ntramp; i++) {
		/* 源 = 迁移单元起点（arm64 含体前 12B prefix）。t->size 即单元大小。*/
		const void *src = (const u8 *)ikaslr_tbl[i]->body -
				  IKASLR_PREFIX_BYTES;

		memcpy(v->base + v->off[i], src, ikaslr_tbl[i]->size);
	}
 ...
 */
/* 第 i 个函数当前代码入口（LIVE 变体内），未随机化前为映像母本入口。*/
unsigned long ikaslr_func_addr(int i)
{
	...
	if (ikaslr_live)
		return (unsigned long)ikaslr_live->base + ikaslr_live->off[i] +
		       IKASLR_PREFIX_BYTES;
	return (unsigned long)ikaslr_tbl[i]->body;
#else
	return (unsigned long)READ_ONCE(*ikaslr_tbl[i]->target);
#endif
}
...
int ikaslr_rerandomize(void)
{
	...
	if (IS_ENABLED(CONFIG_IKASLR_DEBUG)) {
		for (i = 0; i < ikaslr_ntramp; i++) {
			const u8 *want = ikaslr_tbl[i]->body;	/* 映像体入口 */
			const u8 *got = (u8 *)next->base + next->off[i] +
					IKASLR_PREFIX_BYTES;	/* 变体体入口 */
